...
typedef struct _INJECT_PROCESSID_LIST {			//注入列表資訊
	LIST_ENTRY	link;
	HANDLE pid;
	BOOLEAN	inject;
}INJECT_PROCESSID_LIST, *PINJECT_PROCESSID_LIST;

typedef struct _INJECT_PROCESSID_DATA {			//注入程序數據資訊
	HANDLE	pid;
	PVOID	imagebase;
	SIZE_T	imagesize;
}INJECT_PROCESSID_DATA, *PINJECT_PROCESSID_DATA;

typedef struct _INJECT_PROCESSID_DLL {			//記憶體載入DLL資訊
	PVOID	x64dll;
	ULONG	x64dllsize;
	PVOID	x86dll;
	ULONG	x86dllsize;
	...
	_In_ PVOID StartContext)
{
	...
	//
	//1.attach程序，2.找導出表ZwContinue 3.組合shellcode 4.申請記憶體  5.Hook ZwContinue 
	//
...
)
{
	...
	PINJECT_PROCESSID_DATA	injectdata = (PINJECT_PROCESSID_DATA)\
		ExAllocateFromNPagedLookasideList(&g_injectDataLookaside); //分配記憶體
...
)
{
	...
	if (Create)
	{
		DPRINT("AddInjectList -> pid:%d\n", ProcessId);
		AddInjectList(ProcessId);
	}
	else
	{
		DPRINT("DeleteInjectList -> pid:%d\n", ProcessId);
...
{
	DPRINT("[DeugMessage] DriverUnload bye~\r\n");
